          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--text)' }}>{data.totalViews}</div>
            ...
            {data.dailyViews?.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <h4 style={{ margin: '0 0 10px', fontSize: 13, color: 'var(--text-muted)', fontWeight: 500 }}>
                  Views Over Time
                </h4>
                <div style={{
                  ...
                  {data.dailyViews.slice(-30).map((d, i) => (
                    <div
                      ...
                      onMouseLeave={e => { e.currentTarget.style.opacity = `${0.7 + (d.count / maxDaily) * 0.3}` }}
                    />
                  ))}
                </div>
            ...
            {Object.keys(data.byToken || {}).length > 0 && (
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {Object.entries(data.byToken).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([token, count]) => (
                    <div key={token} style={{
                      display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                      padding: '6px 10px', background: 'var(--bg-secondary)', borderRadius: 6, fontSize: 13,
                    }}>
