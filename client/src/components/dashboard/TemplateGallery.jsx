...
const CATEGORY_GROUPS = [
  {
    label: 'Kỹ thuật',
    categories: [
      'digital-electronics', 'microprocessor', 'circuit-theory',
      'electronics', 'automation', 'electrical',
      'measurement', 'power-electronics', 'mechanical',
      'technical-drawing', 'fluid-mechanics',
      'computer-science', 'physics', 'mathematics', 'signal-processing'
    ],
  },
  {
    label: 'Chung',
...
export default function TemplateGallery({ onSelectTemplate, onClose }) {
  ...
  const [favorites, setFavorites] = useState(() => {
    try { return JSON.parse(localStorage.getItem('navslides-favorite-templates') || '[]'); } catch { return []; }
  });
  ...
  const difficultyColor = (d) => ({
    basic: '#00ff87', intermediate: '#ffd700', advanced: '#ff4757'
  }[d] || 'var(--text-muted)');

  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" style={{ width: 960, maxHeight: '88vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ width: 210, borderRight: '1px solid var(--border)', overflowY: 'auto', padding: '12px 0', flexShrink: 0 }}>
            <button
              ...
              style={{ width: '90%', margin: '0 auto 6px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', fontSize: 13 }}
              onClick={() => setActiveCategory(null)}
            >
              <span>Tất cả</span>
              <span style={{ fontSize: 10, opacity: 0.7, background: 'var(--bg-hover)', padding: '1px 6px', borderRadius: 8 }}>
                {data.templates.length}
              </span>
            </button>
            <button
              className={`btn ${activeCategory === 'favorites' ? 'btn-primary' : 'btn-secondary'}`}
              style={{ width: '90%', margin: '0 auto 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', fontSize: 13 }}
              ...
              return (
                <div key={group.label}>
                  <div style={{
                    ...
                    padding: '10px 16px 4px', textTransform: 'uppercase', letterSpacing: 1,
                  }}>
                    {group.label}
                  </div>
                  {groupCats.map(cat => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          width: '90%', margin: '0 auto 3px', display: 'flex',
                          alignItems: 'center', gap: 6, padding: '6px 10px', fontSize: 12,
                        }}
                      ...
                      >
                        ...
                        {count > 0 && (
                          <span style={{
                            ...
                            padding: '1px 5px', borderRadius: 8, color: 'var(--text-muted)',
                          }}>
                            {count}
                          </span>
                        )}
                      ...
                      <div style={{ height: 110, position: 'relative', ...bgStyle }}>
                        ...
                        <div style={{ position: 'absolute', bottom: 6, left: 6, display: 'flex', gap: 4 }}>
                          ...
                          <span style={{
                            ...
                            background: 'rgba(0,0,0,0.5)', color: '#ffffff90', backdropFilter: 'blur(4px)',
                          }}>
                        ...
                        <button 
                          ...
                          style={{ position: 'absolute', top: 6, right: 6, color: favorites.includes(template.id) ? '#fbbf24' : 'rgba(255,255,255,0.5)', zIndex: 10 }}
                        >
                          <Star size={16} fill={favorites.includes(template.id) ? '#fbbf24' : 'none'} />
                        </button>
                      </div>
                      <div style={{ padding: 10 }}>
                        <h4 style={{ margin: '0 0 4px', fontSize: 13, lineHeight: 1.3 }}>{template.title}</h4>
                        <p style={{
