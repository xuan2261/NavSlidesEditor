...
import { Sparkles, Loader2, FileText, Edit3, Check } from 'lucide-react'
...
export default function AIGeneratorModal({ onCreatePresentation, onClose }) {
  ...
  return (
    <ModalShell
      ...
      titleId="ai-generator-modal-title"
      size="lg"
      onClose={handleClose}
    >
      <div className="flex items-center gap-2 text-sm font-medium text-text-muted mb-4">
        <FileText size={18} /> Generate a structured deck outline
      </div>
      ...
      {!outline ? (
        <>
          ...
          {error && (
            <div className="text-danger text-[13px] mt-2" role="alert">
              {error}
            </div>
          )}
        </>
        ...
        <>
          ...
          {error && (
            <div className="text-danger text-[13px] mb-2" role="alert">
              {error}
            </div>
          )}
