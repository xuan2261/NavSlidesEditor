  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm font-medium text-text-muted mb-4">
        <Link2 size={18} /> Manage links and embeds
      </div>
      {error && (
        <div
          className="mb-3 rounded-md border border-danger/30 bg-danger/10 px-3 py-2 text-[13px] text-danger"
          role="alert"
        >
          {error}
        </div>
      ...
      {statusMessage && !error && (
        <div
          className="mb-3 rounded-md border border-accent/30 bg-accent/10 px-3 py-2 text-[13px] text-text-primary"
          role="status"
        >
          {statusMessage}
        </div>
