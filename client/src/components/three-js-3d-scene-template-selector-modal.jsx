...
export default function ThreeJs3DSceneTemplateSelectorModal({ onInsert, onClose }) {
  ...
  return (
    <ModalShell
      ...
      titleId="three-js-scene-modal-title"
      size="2xl"
      className="w-[820px] max-h-[88vh]"
      bodyClassName="flex-1 p-0 overflow-hidden"
      onClose={onClose}
      footer={
        <div className="flex justify-end gap-2">
          <Button variant="secondary" onClick={onClose} className="px-4 py-1.5 text-sm">
            Cancel
          </Button>
          <Button variant="primary" onClick={handleInsert} className="px-4 py-1.5 text-sm">
            Insert
          </Button>
        </div>
      }
          ...
          {TEMPLATES.map((t) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded text-sm mb-1 cursor-pointer ${
                selected === t.id ? 'bg-accent text-white' : 'hover:bg-hover text-text-primary'
              }`}
