...
export default function FileBrowserModal({ presentationId, onInsert, onClose }) {
  ...
  const filtered =
    filter === 'all' ? files : files.filter((f) => f.type?.startsWith(filter))
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div
        className="bg-card border border-border rounded-lg shadow-xl w-[640px] max-h-[70vh] flex flex-col"
      ...
      >
        ...
        <div className="p-4 border-t border-border flex justify-end">
          <button className="btn btn-secondary px-4 py-1.5 text-sm" onClick={onClose}>
            Close
          </button>
        </div>
      </div>
