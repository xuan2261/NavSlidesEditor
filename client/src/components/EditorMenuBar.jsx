...
import {
  Download, FileDown, Github, CloudUpload, History,
  Search, Clock, Code2, MessageSquare,
  Share2, Radio, BarChart3,
  Sparkles, FileText, Languages,
  Play, FolderOpen,
  LayoutGrid,
} from 'lucide-react'
...
export default function EditorMenuBar({
  ...
  const fileItems = [
    ...
    { type: 'button', label: 'Export Project (.navslides)', icon: FileDown, onClick: onExportProject },
    { type: 'separator' },
  ...
  const viewItems = [
    {
      type: 'button', label: 'Find & Replace', icon: Search,
      shortcut: 'Ctrl+F', onClick: onFindReplace,
    },
    {
      type: 'button', label: 'Animation Timeline', icon: Clock,
      onClick: onTimeline,
    },
    ...
    { type: 'separator' },
    { type: 'button', label: 'Slide Sorter', icon: LayoutGrid, shortcut: 'Ctrl+Shift+S', onClick: onSlideSorter },
  ]
  ...
  const settingsItems = [
    ...
    {
      type: 'select', label: 'Slide Size',
      value: presentation.resolution
        ? `${presentation.resolution.width}x${presentation.resolution.height}` : '960x540',
      onChange: (v) => {
        if (v === 'custom') {
    ...
    { type: 'separator' },
    {
      type: 'checkbox', label: 'Auto-advance',
    ...
    ...(presentation.autoSlide ? [
      {
        type: 'custom',
        render: () => (
          <div className="dropdown-item dropdown-select-row">
            <span>Interval</span>
    ...
    {
      type: 'checkbox', label: 'Dark / Light Toggle',
      checked: presentation.presenterTools?.themeToggle !== false,
      onChange: (v) => setPresentation((p) => ({
        ...p,
        presenterTools: { ...p.presenterTools, themeToggle: v },
      })),
    },
    {
      type: 'checkbox', label: 'Font Size Zoom (A+/A-)',
    ...
    {
      type: 'checkbox', label: 'Slide Menu & Tools',
    ...
    {
      type: 'checkbox', label: 'Pen / Chalkboard',
  ...
  return (
    <div className="editor-menu-bar tour-step-quick-access">
      ...
      {saveStatus === 'saved' && (
        <span className="save-indicator" style={{ color: 'var(--success)' }}>Saved</span>
      )}
      ...
      <span className="menu-bar-divider" />
      ...
      {/* Primary CTA */}
      <button className="btn btn-primary" onClick={onPresent} title="Present">
        <Play size={14} />
        Present
      </button>
      ...
      {customSizePrompt && (
        <PromptPopover
          ...
          defaultValue={presentation.resolution
