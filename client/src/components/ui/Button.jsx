...
export function buttonVariants({ variant = 'primary', className = '' }) {
  const baseClasses =
    'inline-flex items-center justify-center gap-[6px] px-[14px] py-[7px] rounded-md text-[13px] font-medium transition-all duration-150 ease-out cursor-pointer border-none active:scale-[0.97] disabled:opacity-50 disabled:pointer-events-none'
...
export const Button = React.forwardRef(
  ({ className, variant = 'primary', type = 'button', ...props }, ref) => {
    return (
      <button type={type} className={buttonVariants({ variant, className })} ref={ref} {...props} />
    )
  }
)
