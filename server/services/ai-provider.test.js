...
describe('AI Provider', () => {
  ...
  it('should route to Gemini and format request correctly', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('generativelanguage.googleapis.com'), expect.objectContaining({
      method: 'POST'
    }));
  });
  ...
  it('should throw on API mismatch or failure', async () => {
    ...
    await expect(callAI(config, 'system', 'user')).rejects.toThrow('OpenAI API Error: Invalid API key');
  });
});
