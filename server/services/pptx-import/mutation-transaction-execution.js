...
function blocked(reasonCode) { return frozen({ ok: false, status: 422, blockReason: reasonCode, reasonCode }) }
function conflict(currentGeneration) { return frozen({ ok: false, status: 409, conflict: frozen({ type: 'STALE_GENERATION', currentGeneration }) }) }
...
function idempotencyResult(state, request, hash) {
  ...
  if (prior.requestHash !== hash) return frozen({ ok: false, status: 409, reasonCode: 'IDEMPOTENCY_KEY_CONFLICT', blockReason: 'IDEMPOTENCY_KEY_CONFLICT' })
  return prior
}
...
async function executeLocked(dependencies, request) {
  ...
  if (!head) throw new Error('Presentation package head is missing')
  ...
  if (!validateMatrixAuthoritySubjects(head.matrixAuthoritySubjects).authorized) {
    return blocked('STALE_MATRIX_AUTHORITY')
  }
  if (request.baseRevisionId !== undefined && request.baseRevisionId !== head.packageRevisionId) {
    return frozen({ ok: false, status: 409, conflict: frozen({ type: 'BASE_REVISION_MISMATCH', currentGeneration: head.generation, currentRevisionId: head.packageRevisionId }) })
  }
  ...
  let journal
  const journalOptions = { baseRevisionId: head.packageRevisionId, sourceMap, requestIdentity: hash }
  ...
  const plan = compilePatchPlan(journal); if (!plan.ok) return blocked(plan.reasonCode)
  if (request.cancelled) return frozen({ ok: false, status: 409, cancellation: 'cancelled' })
