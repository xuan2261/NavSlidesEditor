...
function expectedRevealSlideCount(presentation) {
  return (presentation?.slides || []).reduce(
    (count, slide) => count + 1 + (slide.children?.length || 0),
    0
  )
}
...
async function closeResource(resource, name, timeoutMs) {
  ...
  try {
    ...
    return error?.code === 'capture-timeout'
      ? `capture-${name}-close-timeout`
      : `capture-${name}-close-failed`
  } finally {
    deadline.clear()
  }
}
  ...
  try {
    ...
    if (error?.code === 'capture-timeout') {
      Promise.resolve(pending).then(
        (browser) => closeResource(browser, 'browser', teardownTimeoutMs),
        () => {}
      )
...
function normalizeRevealPlan(plan) {
  ...
  return plan.map((target, ordinal) => {
    if (
      ...
      !Number.isSafeInteger(target.h) ||
      target.h < 0 ||
      !Number.isSafeInteger(target.v) ||
      target.v < 0
    ) {
...
async function captureRevealSlides(
  page,
  ...
  for (const target of plan) {
    await abortable(signal, () =>
      page.evaluate(({ h, v }) => window.Reveal.slide(h, v, -1), target)
    )
    await abortable(signal, () =>
      page.waitForFunction((expected) => {
        ...
        const animations =
          typeof current?.getAnimations === 'function'
            ? current.getAnimations({ subtree: true })
            : []
        return (
          indices.h === expected.h &&
          (indices.v ?? 0) === expected.v &&
          current === reveal.getSlides()[expected.ordinal] &&
          animations.every((animation) => ['finished', 'idle'].includes(animation.playState))
        )
      }, target)
    )
    const currentHandle = await abortable(signal, () =>
      page.evaluateHandle(() => window.Reveal.getCurrentSlide())
    )
...
function withAssetBase(html, assetBaseUrl) {
  ...
  if (!['http:', 'https:'].includes(base.protocol))
    throw new Error('invalid-present-assets-base-url')
...
async function capturePresentSlides(presentation, options = {}) {
  ...
  if (!Number.isSafeInteger(teardownTimeoutMs) || teardownTimeoutMs < 1)
    return { ok: false, files: [], error: 'invalid-teardown-timeout' }
  ...
  try {
    ...
    page = await abortable(options.signal, () =>
      browser.newPage({ viewport: VIEWPORT, deviceScaleFactor: 1 })
    )
    ...
    await abortable(options.signal, () =>
      page.setContent(withAssetBase(html, assetBaseUrl), { waitUntil: 'load' })
    )
    await abortable(options.signal, () =>
      page.waitForFunction(() => window.Reveal?.isReady?.() === true)
    )
    await abortable(options.signal, () =>
      page.evaluate(
        ({ revealConfig, viewport }) => {
    ...
    const capture = await captureRevealSlides(page, {
      deckDir: path.join(outDir, deckStem),
      expectedSlideCount,
