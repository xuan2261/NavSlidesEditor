function auditCollection(state) {
  ...
  const quarantined = new Set((state.candidateBlobs || []).map((candidate) => candidate.sha256))
  const candidates = state.blobs
    .filter((blob) => !referenced.has(blob.sha256))
    .filter((blob) => !quarantined.has(blob.sha256))
...
function auditBlobFiles(state, files) {
  const known = new Set([
    ...state.blobs.map((blob) => `${blob.sha256}.blob`),
    ...(state.candidateBlobs || []).map((candidate) => `${candidate.sha256}.blob`),
  ])
  return { schemaVersion: 1, mode: 'audit-only', physicalDeletionEnabled: false,
    ...
    quarantined: (state.candidateBlobs || []).map((candidate) => candidate.sha256).sort(),
  }
}
