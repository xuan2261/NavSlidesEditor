...
async function withFileLock(filePath, fn) {
  ...
  fileLocks.set(filePath, prev.then(() => lockPromise));
  await prev;
  try {
    return await fn();
  } finally {
...
function initDataFiles() {
  ...
  if (!fs.existsSync(SETTINGS_FILE)) {
    fs.writeJsonSync(SETTINGS_FILE, { aiApiKey: '', defaultTheme: 'black', defaultTransition: 'slide' });
  }
}
...
async function writeTemplates(data) {
  return fs.writeJson(TEMPLATES_FILE, data, { spaces: 2 });
}

// ── Share Tokens ─────────────────────────────────────────────────────────────
async function readShareTokens() {
  return fs.readJson(SHARE_FILE);
}

async function writeShareTokens(data) {
  return fs.writeJson(SHARE_FILE, data, { spaces: 2 });
}

// ── GitHub Config ────────────────────────────────────────────────────────────
async function readGithubConfig() {
  return fs.readJson(GITHUB_CONFIG_FILE);
}

async function writeGithubConfig(data) {
  return fs.writeJson(GITHUB_CONFIG_FILE, data, { spaces: 2 });
}
...
async function writeSettings(data) {
  return fs.writeJson(SETTINGS_FILE, data, { spaces: 2 });
}

module.exports = {
  // Paths
  DATA_DIR, UPLOADS_DIR, SHARE_FILE, TEMPLATES_FILE, HISTORY_DIR,
  GITHUB_CONFIG_FILE, RCLONE_CONFIG_FILE, SYNC_DIR, SETTINGS_FILE,
  ...
  readShareTokens, writeShareTokens,
  readGithubConfig, writeGithubConfig,
