...
const {
  readPresentations, RCLONE_CONFIG_FILE, SYNC_DIR, UPLOADS_DIR,
...
router.get('/status', async (req, res) => {
  try {
    let installed = false;
    let version = '';
    try { version = await runRclone(['version']); installed = true; } catch {}
...
router.post('/config', async (req, res) => {
  try {
    ...
    try { await runRclone(['lsd', `${name}:`]); } catch (err) {
