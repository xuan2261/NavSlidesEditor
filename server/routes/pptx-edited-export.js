...
function createEditedExportHandler({ findPresentation, getAvailability, execute }) {
  return async function editedExport(req, res) {
    try {
      ...
      const availability = await getAvailability(presentation)
      if (!availability.available) {
        return safeError(
          res, 422, availability.reasonCode || 'EDITED_EXPORT_UNAVAILABLE',
          'Validated edited export is unavailable'
        )
      }
      const idempotencyKey = req.get('Idempotency-Key')
      const expectedGeneration = Number(req.get('If-Pptx-Generation'))
      if (!idempotencyKey || !Number.isSafeInteger(expectedGeneration) || expectedGeneration < 0) {
