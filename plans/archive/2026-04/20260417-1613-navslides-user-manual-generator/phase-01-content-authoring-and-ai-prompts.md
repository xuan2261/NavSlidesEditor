...
3. Soạn thảo chi tiết 3 mẫu Prompt Master-level:
   - _Prompt kỹ thuật:_ Ép LLM dùng `style="..."` thay vì `<style>` ngoài để tránh xung đột CSS.
   - _Prompt mô phỏng:_ Yêu cầu dùng vanilla JS với `requestAnimationFrame` và giới hạn kích thước vùng vẽ (`960x700`).
