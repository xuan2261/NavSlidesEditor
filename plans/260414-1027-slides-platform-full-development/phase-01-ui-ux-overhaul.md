...
```css
/* Card hover */
.presentation-card { transition: transform var(--transition-fast), box-shadow var(--transition-fast); }
.presentation-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-lg); }

/* Button press */
.btn { transition: transform 100ms ease, background var(--transition-fast); }
.btn:active { transform: scale(0.97); }

/* Panel slide */
.panel-enter { animation: slideIn 200ms ease-out; }
@keyframes slideIn { from { opacity: 0; transform: translateX(12px); } }

/* Tooltip fade */
.tooltip { animation: fadeIn 150ms ease; }
```
...
.label { font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-muted); }
```
