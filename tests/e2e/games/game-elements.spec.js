...
test.describe('Game Element — Toolbar Integration', () => {
  ...
  test('Insert menu shows all 7 game types', async ({ page }) => {
    ...
    await expect(page.locator('.insert-dropdown')).toBeVisible({ timeout: 5000 })
    await expect(page.getByText('Games')).toBeVisible({ timeout: 3000 })
    await expect(page.getByText('Name Picker')).toBeVisible()
    await expect(page.getByText('Hot Potato Quiz')).toBeVisible()
    await expect(page.getByText('Jeopardy')).toBeVisible()
    await expect(page.getByText('Four Corners')).toBeVisible()
    await expect(page.getByText('Relay Race')).toBeVisible()
    await expect(page.getByText('Trivia Championship')).toBeVisible()
    await expect(page.getByText('Scattergories')).toBeVisible()
  })
})
