export class HomePage {
  ...
  constructor(page) {
    this.page = page;
    this.newPresentationBtn = page.locator('.btn-primary:has-text("New")');
    this.modalTitleInput = page.locator('input[placeholder="My Presentation"]');
    this.modalCreateBtn = page.locator('.modal-footer button.btn-primary:has-text("Create")');
    this.searchInput = page.locator('.home-search-input');
    this.presentationCards = page.locator('.presentation-card');
  }

  async goto() {
    await this.page.goto('/', { timeout: 30000 });
    // Chờ Vite compile và render react app
    await this.page.waitForSelector('.home-page', { timeout: 30000 });
  }
  ...
  async switchSidebarView(viewLabel) {
    await this.page.locator('.sidebar-item').filter({ hasText: viewLabel }).click();
    await this.page.waitForTimeout(500);
  }
  ...
  async navigateToExplore() {
    await this.page.locator('.sidebar-item').filter({ hasText: 'Explore' }).click();
  ...
  async createFromPresetTemplate(templateName) {
    ...
    await this.page.locator('.presentation-card').filter({ hasText: templateName }).click();
    await this.page.waitForURL(/\/editor\/.+/, { timeout: 30000 });
  }
