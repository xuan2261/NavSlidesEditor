export class EditorPage {
  ...
  constructor(page) {
    this.page = page;
    this.canvasLocator = page.locator('.slide-canvas-wrapper');
  ...
  async clickInsertMenuItem(itemName) {
    const insertMenuOpen = await this.page.locator('.insert-dropdown').isVisible().catch(() => false);
    if (!insertMenuOpen) {
  ...
  async addSlide() {
    ...
    await this.page.waitForSelector('.modal h2:has-text("Add Slide")');
    await this.page.locator('.modal button').filter({ hasText: 'Blank' }).click();
    await this.page.waitForFunction((prev) => {
  ...
  async addSlideFromTemplate(templateName) {
    ...
    await this.addSlideBtn.click();
    await this.page.waitForSelector('.modal h2:has-text("Add Slide")');
    await this.page.locator('.modal button').filter({ hasText: templateName }).click();
    await this.page.waitForFunction((prev) => {
      return document.querySelectorAll('.slide-panel .slide-item').length > prev;
    }, prevCount, { timeout: 5000 });
  }
  ...
  async changeBackgroundToGradient() {
    ...
    const initialBgColor = await this.page.evaluate(() => {
      const el = document.querySelector('.slide-canvas-wrapper');
    ...
    if (await swatches.count() > 0) {
      ...
      }, initialBgColor, { timeout: 5000 }).catch(() => {});
    }
  }
  ...
  async closeFindReplace() {
    ...
    await this.page.waitForSelector('.find-replace-bar', { state: 'hidden', timeout: 5000 }).catch(() => {});
  }
  ...
  async addTable(rows = 3, cols = 3) {
    ...
    await this.page.locator('.table-size-picker .table-cell').nth((rows - 1) * 8 + (cols - 1)).click();
    await this.page.waitForFunction((prev) => {
      return document.querySelectorAll('.element-wrapper').length > prev;
    }, prevCount, { timeout: 5000 });
  }
