...
test.describe('axe core scans across editor present share live and home views with stable DOM wait', () => {
  ...
  test('editor page emits no critical axe violations beyond known baseline', async ({ page }) => {
    await page.goto(`/editor/${presId}`)
    await page.waitForSelector('.slide-canvas', { timeout: 15000 })
    const { critical } = await scanA11y(page, 'editor', { disableRules: SHARED_DISABLE_RULES })
    const fresh = newCriticalViolations(critical, 'editor')
  ...
  test('present preview page emits zero critical axe violations', async ({ page }) => {
    await page.goto(`/api/presentations/${presId}/present?preview=true`)
    await page.waitForSelector('.reveal section', { timeout: 15000 })
    const { critical } = await scanA11y(page, 'present', { disableRules: SHARED_DISABLE_RULES })
    const fresh = newCriticalViolations(critical, 'present')
    if (fresh.length) console.log('[axe present NEW critical]', JSON.stringify(fresh.map((c) => c.id), null, 2))
    expect(fresh).toHaveLength(0)
  })

  test('share landing page emits zero critical axe violations', async ({ page, request }) => {
    const { token } = await apiCreateShareLink(request, presId)
    await page.goto(`/share/${token}`)
    await page.waitForSelector('.reveal section', { timeout: 15000 })
    const { critical } = await scanA11y(page, 'share', { disableRules: SHARED_DISABLE_RULES })
    const fresh = newCriticalViolations(critical, 'share')
    if (fresh.length) console.log('[axe share NEW critical]', JSON.stringify(fresh.map((c) => c.id), null, 2))
    expect(fresh).toHaveLength(0)
  })

  test('live viewer initial state emits zero critical axe violations', async ({ page, request }) => {
  ...
  test('home dashboard emits no critical axe violations beyond known baseline', async ({ page }) => {
    await page.goto('/')
    await page.waitForLoadState('networkidle', { timeout: 15000 })
    const { critical } = await scanA11y(page, 'home', { disableRules: SHARED_DISABLE_RULES })
    const fresh = newCriticalViolations(critical, 'home')
